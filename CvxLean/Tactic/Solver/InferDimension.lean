...
unsafe def inferDimension (ty : Expr) : MetaM (List (Nat × Nat)) :=
  ...
  | Expr.const ``Real _ => 
      return [(1, 1)]
  | Expr.forallE _ (Expr.app (Expr.const ``Finₓ _) nExpr) e _ => 
    match e with 
    | Expr.const ``Real _  => do
        let n : Nat ← evalExpr Nat (mkConst ``Nat) nExpr
        return [(n, 1)]
    | Expr.forallE _ (Expr.app (Expr.const ``Finₓ _) mExpr) (Expr.const ``Real _) _ => do 
        let n : Nat ← evalExpr Nat (mkConst ``Nat) nExpr
        let m : Nat ← evalExpr Nat (mkConst ``Nat) mExpr
        return [(n, m)]
    | _ => throwError "Unsupported type: {ty}"
  | Expr.app (Expr.app (Expr.app M FinN) FinM) R => do 
    match (M, FinM, FinM, R) with 
    | (Expr.const ``Matrix _, 
      Expr.app (Expr.const ``Finₓ _) nExpr, 
      Expr.app (Expr.const ``Finₓ _) mExpr,
      Expr.const ``Real _) => 
        let n : Nat ← evalExpr Nat (mkConst ``Nat) nExpr
        let m : Nat ← evalExpr Nat (mkConst ``Nat) mExpr
        return [(n, m)]
