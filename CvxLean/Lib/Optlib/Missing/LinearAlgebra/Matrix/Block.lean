...
lemma BlockTriangular_blockDiagonal' [DecidableEq α] (d : ∀ (i : α), Matrix (m' i) (m' i) R) :
...
lemma toBlock_inverse_mul_toBlock_eq_one_of_BlockTriangular [LinearOrder α]
  ...
  M⁻¹.toBlock (fun i => b i < k) (fun i => b i < k) ⬝ M.toBlock (fun i => b i < k) (fun i => b i < k) = 1 := by
  let p := (fun i => b i < k)
...
lemma inv_toBlock_of_BlockTriangular [LinearOrder α]
  [Invertible M] (hM : BlockTriangular M b) (k : α) :
  (M.toBlock (fun i => b i < k) (fun i => b i < k))⁻¹ = M⁻¹.toBlock (fun i => b i < k) (fun i => b i < k) :=
inv_eq_left_inv (toBlock_inverse_mul_toBlock_eq_one_of_BlockTriangular hM k)
...
invertibleOfLeftInverse _ ((⅟M).toBlock (fun i => b i < k) (fun i => b i < k))
  (by simpa only [invOf_eq_nonsing_inv]
    using toBlock_inverse_mul_toBlock_eq_one_of_BlockTriangular hM k)
...
lemma toSquareBlock_inv_mul_toSquareBlock_eq_one [LinearOrder α]
  ...
  have h_sum : M⁻¹.toBlock p p ⬝ M.toBlock p p +
    ...
    rw [←toBlock_mul_eq_add, inv_mul_of_invertible M, toBlock_one_self]
  have h_zero :
    ∀ i j l, M⁻¹.toBlock p (λ (i : m) => ¬p i) i j * M.toBlock (λ (i : m) => ¬p i) p j l = 0
  { intro i j l
    by_cases hj : b j.1 ≤ k
  ...
  have h_zero' : M⁻¹.toBlock p (λ (i : m) => ¬p i) ⬝ M.toBlock (λ (i : m) => ¬p i) p = 0
  { ext i l
