...
@[termElab prodField] def elabProdField : Term.TermElab := 
...
where aux (top : Expr) (first : Option Bool := none) : DelabM (Syntax × Nat) := do
  ...
  | none, Expr.app (Expr.app (Expr.app (Expr.const ``Prod.fst _) α) β) x => do
    withNaryArg 2 do aux top true
  | _, Expr.app (Expr.app (Expr.app (Expr.const ``Prod.snd _) α) β) x => do
    withNaryArg 2 do
